...
def test_parse_sudo_command_line() -> None:
    """A sudo command record should parse into a sudo_command event."""
    line = (
        "Mar 25 10:05:00 debian sudo:   alice : TTY=pts/0 ; "
        "PWD=/home/alice ; USER=root ; COMMAND=/usr/bin/cat /etc/shadow"
    )
    event = parse_sudo_line(line, year=2026)
    assert event is not None
    assert event.source == "sudo"
    assert event.event_type == "sudo_command"
    assert event.username == "alice"
    assert event.hostname == "debian"
    assert event.path == "/usr/bin/cat /etc/shadow"


def test_parse_sudo_incorrect_password_attempts() -> None:
    """A sudo record with failed attempts should get its own event type."""
    line = (
        "Mar 25 10:05:01 debian sudo:   alice : 3 incorrect password "
        "attempts ; TTY=pts/0 ; PWD=/home/alice ; USER=root ; "
        "COMMAND=/usr/bin/id"
    )
    event = parse_sudo_line(line, year=2026)
    assert event is not None
    assert event.event_type == "sudo_incorrect_password_attempts"
    assert event.username == "alice"


def test_parse_sudo_pam_auth_failure() -> None:
    """A sudo PAM authentication failure should parse with the username."""
    line = (
        "Mar 25 10:05:02 debian sudo: pam_unix(sudo:auth): authentication "
        "failure; logname=alice uid=1000 euid=0 tty=/dev/pts/0 ruser=alice "
        "rhost=  user=alice"
    )
    event = parse_sudo_line(line, year=2026)
    assert event is not None
    assert event.event_type == "sudo_auth_failure"
    assert event.username == "alice"


def test_parse_sudo_line_rejects_unrelated_lines() -> None:
    """sshd or empty lines should not parse as sudo events."""
    assert parse_sudo_line("") is None
    assert (
        parse_sudo_line(
            "Mar 25 10:00:01 debian sshd[2001]: Failed password for root "
            "from 185.10.10.1 port 40001 ssh2",
            year=2026,
        )
        is None
    )
...
def test_parse_auth_line_ignores_sudo_lines() -> None:
    """Sudo records must not be mis-parsed as ssh auth events."""
    line = (
        "Mar 25 10:05:00 debian sudo:   alice : TTY=pts/0 ; "
        "PWD=/home/alice ; USER=root ; COMMAND=/usr/bin/id"
    )
    assert parse_auth_line(line, year=2026) is None


def test_parse_auth_line_gdm_password_pam_failure() -> None:
    """A gdm-password PAM failure should parse as pam_auth_failure."""
    line = (
        "Mar 25 10:05:03 debian gdm-password[1234]: "
        "pam_unix(gdm-password:auth): authentication failure; logname= uid=0 "
        "euid=0 tty=/dev/tty1 ruser= rhost=  user=alice"
    )
    event = parse_auth_line(line, year=2026)
    assert event is not None
    assert event.source == "auth"
    assert event.event_type == "pam_auth_failure"
    assert event.service == "gdm-password"
    assert event.username == "alice"
    assert event.src_ip is None
    assert event.hostname == "debian"
    assert event.process == "gdm-password"


def test_parse_auth_line_login_service_pam_failure() -> None:
    """A TTY login PAM failure should parse with the login service."""
    line = (
        "Mar 25 10:05:04 debian login[987]: pam_unix(login:auth): "
        "authentication failure; logname= uid=0 euid=0 tty=/dev/tty2 "
        "ruser= rhost=  user=bob"
    )
    event = parse_auth_line(line, year=2026)
    assert event is not None
    assert event.event_type == "pam_auth_failure"
    assert event.service == "login"
    assert event.username == "bob"


def test_parse_auth_line_ignores_sudo_pam_failure() -> None:
    """sudo PAM failures belong to parse_sudo_line, not parse_auth_line."""
    line = (
        "Mar 25 10:05:02 debian sudo: pam_unix(sudo:auth): authentication "
        "failure; logname=alice uid=1000 euid=0 tty=/dev/pts/0 ruser=alice "
        "rhost=  user=alice"
...
def test_parse_auth_line_rejects_malformed_pam_lines() -> None:
    """PAM lines without a user= field or with garbage should not parse."""
    assert (
        parse_auth_line(
            "Mar 25 10:05:03 debian gdm-password[1234]: "
    ...
    assert parse_auth_line("", year=2026) is None
    assert (
        parse_auth_line(
            "Mar 25 10:05:03 debian gdm-password[1234]: some other message",
            year=2026,
        )
        is None
    )
