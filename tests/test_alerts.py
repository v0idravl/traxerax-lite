...
class _FakePath(str):
    """str-based Path stand-in with a configurable exists() result."""
...
def _stub_notify_binaries(monkeypatch) -> list:
    """Stub binary resolution and subprocess; return captured commands."""
    monkeypatch.setattr(
...
def test_desktop_notification_wraps_runuser_for_notify_user_as_root(
    monkeypatch,
) -> None:
    """As root with notify_user set, notify-send runs as that user on their bus."""
    ...
    monkeypatch.setattr("traxerax_lite.alerts.Path", _FakePath)
    calls = _stub_notify_binaries(monkeypatch)

    assert (
        send_desktop_notification("summary", "high", _logger(), notify_user="alice")
        is True
    )
    command = calls[0]
    assert command[:4] == ["/usr/bin/runuser", "-u", "alice", "--"]
    assert command[4] == "/usr/bin/env"
    assert command[5] == (
        "DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus"
    )
    assert command[6] == "/usr/bin/notify-send"


def test_desktop_notification_ignores_notify_user_when_not_root(
    monkeypatch,
) -> None:
    """A non-root process should notify directly even when notify_user is set."""
    monkeypatch.setattr("os.geteuid", lambda: 1000)
    calls = _stub_notify_binaries(monkeypatch)

    assert (
        send_desktop_notification("summary", "low", _logger(), notify_user="alice")
        is True
    )
    assert calls[0][0] == "/usr/bin/notify-send"


def test_desktop_notification_skips_when_session_bus_missing(monkeypatch) -> None:
    """No session bus for notify_user should degrade to False, never raise."""
    monkeypatch.setattr("os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "pwd.getpwnam",
        lambda name: type("PW", (), {"pw_uid": 1000})(),
