...
def _sudo_failure(user, minute):
    return Event(
        timestamp=datetime(2026, 3, 25, 10, minute, 0),
        source="sudo",
        event_type="sudo_auth_failure",
        raw=f"sudo: pam_unix(sudo:auth): authentication failure; user={user}",
        username=user,
        service="sudo",
        hostname="debian",
        process="sudo",
...
def test_repeated_sudo_auth_failures_trigger_finding() -> None:
    """Three sudo auth failures for one user should raise one finding."""
    state = DetectionState()
    findings = []
    for minute in range(3):
        findings.extend(process_event(_sudo_failure("alice", minute), state))
    assert len(findings) == 1
    assert findings[0].finding_type == "repeated_sudo_auth_failures"
    assert findings[0].severity == "medium"
    assert findings[0].src_ip is None
    assert "alice" in findings[0].message


def test_sudo_failures_below_threshold_stay_silent() -> None:
    """Two sudo auth failures should not trigger the threshold finding."""
    state = DetectionState()
    findings = []
...
def test_sudo_failures_are_tracked_per_user() -> None:
    """Failures from different users must not combine across the threshold."""
    state = DetectionState()
    findings = []
    for user in ("alice", "bob", "carol"):
        findings.extend(process_event(_sudo_failure(user, 0), state))
    assert findings == []


def test_sudo_finding_respects_disabled_rule() -> None:
    """A disabled repeated_sudo_auth_failures rule suppresses the finding."""
    state = DetectionState()
    state.enabled_rules["repeated_sudo_auth_failures"] = False
    findings = []
...
def test_sudo_command_events_produce_no_findings() -> None:
    """Plain sudo command records are visibility-only, never findings."""
    state = DetectionState()
    event = Event(
        timestamp=datetime(2026, 3, 25, 10, 0, 0),
        source="sudo",
        event_type="sudo_command",
        raw="sudo: alice : USER=root ; COMMAND=/usr/bin/id",
        username="alice",
        service="sudo",
        hostname="debian",
        process="sudo",
    ...
    assert process_event(event, state) == []


def _pam_failure(user, minute, service="gdm-password"):
    return Event(
        timestamp=datetime(2026, 3, 25, 10, minute, 0),
        source="auth",
        event_type="pam_auth_failure",
        raw=f"{service}: pam_unix({service}:auth): authentication failure; user={user}",
        username=user,
        service=service,
        hostname="debian",
        process=service,
...
def test_repeated_pam_auth_failures_trigger_local_finding() -> None:
    """Three gdm-password failures for one user should raise one finding."""
    state = DetectionState()
    findings = []
    for minute in range(3):
        findings.extend(process_event(_pam_failure("alice", minute), state))
    assert len(findings) == 1
    assert findings[0].finding_type == "repeated_local_auth_failures"
    assert findings[0].severity == "medium"
    assert findings[0].src_ip is None
    assert "gdm-password" in findings[0].message
    assert "alice" in findings[0].message


def test_pam_auth_failures_below_threshold_stay_silent() -> None:
    """Two local PAM failures should not trigger the threshold finding."""
    state = DetectionState()
    findings = []
    for minute in range(2):
        findings.extend(process_event(_pam_failure("alice", minute), state))
...
def test_pam_auth_failures_are_tracked_per_user() -> None:
    """Local PAM failures from different users must not combine."""
    state = DetectionState()
    findings = []
    for user in ("alice", "bob", "carol"):
        findings.extend(process_event(_pam_failure(user, 0), state))
    assert findings == []


def test_pam_auth_finding_names_login_service() -> None:
    """TTY login failures should name the login service in the message."""
    state = DetectionState()
    findings = []
    for minute in range(3):
        findings.extend(
            process_event(_pam_failure("bob", minute, service="login"), state)
        )
    assert len(findings) == 1
    assert findings[0].finding_type == "repeated_local_auth_failures"
...
def test_pam_auth_finding_respects_disabled_rule() -> None:
    """A disabled repeated_local_auth_failures rule suppresses the finding."""
    state = DetectionState()
    state.enabled_rules["repeated_local_auth_failures"] = False
    findings = []
    for minute in range(4):
        findings.extend(process_event(_pam_failure("alice", minute), state))
    assert findings == []
...
def test_sudo_and_pam_failures_share_the_user_threshold() -> None:
    """Sudo and local PAM failures for one user combine into one finding."""
    state = DetectionState()
    findings = []
    findings.extend(process_event(_sudo_failure("alice", 0), state))
    findings.extend(process_event(_pam_failure("alice", 1), state))
    assert findings == []
    # The third failure crosses the threshold; the alerting event's service
    # determines which finding type is produced.
    findings.extend(process_event(_pam_failure("alice", 2), state))
    assert len(findings) == 1
    assert findings[0].finding_type == "repeated_local_auth_failures"
