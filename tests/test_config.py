...
def test_load_daemon_settings_run_log_ingestion_default_and_override() -> None:
    """Daemon log ingestion should default on and honor the YAML toggle."""
    assert load_daemon_settings({}).run_log_ingestion is True
    settings = load_daemon_settings({"daemon": {"run_log_ingestion": False}})
    assert settings.run_log_ingestion is False


def test_load_alert_settings_notify_user_default_and_override() -> None:
    """notify_user should default to empty and honor the YAML value."""
    assert load_alert_settings({}).notify_user == ""
    settings = load_alert_settings({"alerts": {"notify_user": "alice"}})
    assert settings.notify_user == "alice"


def test_load_journald_settings_identifiers_default_and_override() -> None:
    """Desktop auth identifiers are default; YAML overrides replace the map."""
    assert load_journald_settings({}).identifiers == {
        "auth": ("sudo", "gdm-password", "sddm-helper", "login")
    }
    settings = load_journald_settings(
        {"journald": {"identifiers": {"auth": ["sudo", "su"]}}}
    )
    assert settings.identifiers == {"auth": ("sudo", "su")}


def test_load_detection_settings_sudo_thresholds() -> None:
    """Sudo detection knobs should default and honor YAML overrides."""
    default = load_detection_settings({})
    assert default.sudo_failed_auth_threshold == 3
    assert default.sudo_failure_window_seconds == 900
    settings = load_detection_settings(
        {
            "detection": {
                "thresholds": {"sudo_failed_auth": 5},
                "windows": {"sudo_failed_auth_seconds": 60},
            }
        }
    )
    assert settings.sudo_failed_auth_threshold == 5
    assert settings.sudo_failure_window_seconds == 60


def test_load_host_settings_enables_new_collectors_by_default() -> None:
    """The persistence/USB/browser collectors should be on by default."""
    ...
    for collector in (
        ...
        assert collector in settings.enabled_collectors


def test_load_change_settings_new_category_defaults() -> None:
    """The new change categories default on with empty ignore lists."""
    settings = load_change_settings({})

    assert settings.xdg_autostart is True
    assert settings.systemd_user_units is True
    assert settings.usb_devices is True
    assert settings.browser_extensions is True
    assert settings.ignored_usb_devices == ()
    assert settings.ignored_browser_extensions == ()


def test_load_change_settings_applies_new_yaml_overrides() -> None:
    """Configured new toggles and ignore lists should override defaults."""
    settings = load_change_settings(
        {
            "changes": {
                "xdg_autostart": False,
                "usb_devices": False,
                "ignored_usb_devices": ["046d:c52b:ABC123"],
                "ignored_browser_extensions": ["alice:chrome:abc"],
            }
        }
    )

    assert settings.xdg_autostart is False
    assert settings.systemd_user_units is True
    assert settings.usb_devices is False
    assert settings.browser_extensions is True
    assert settings.ignored_usb_devices == ("046d:c52b:ABC123",)
    assert settings.ignored_browser_extensions == ("alice:chrome:abc",)


def test_load_audit_settings_enables_snap_and_flatpak_by_default() -> None:
    """snap_confinement and flatpak_permissions default on at medium."""
    settings = load_audit_settings({})

    for check_id in ("snap_confinement", "flatpak_permissions"):
        assert check_id in settings.enabled_checks
        assert settings.check_severities[check_id] == "medium"
