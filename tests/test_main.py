...
""")
    ...
    monkeypatch.setattr(
        ...
        lambda summary, max_severity, logger, notify_user="": (
            notifications.append(summary) or True
        ),
    )
...
def _run_daemon_with_stubbed_scheduler(
    monkeypatch, config_path, db_path, ticks=1
):
    """Run main() in daemon mode with a scheduler that fires fixed ticks."""
    ...
    monkeypatch.setattr("traxerax_lite.main.EBPFLoader", _DaemonStubLoader)
    monkeypatch.setattr("traxerax_lite.main.Scheduler", _StubScheduler)
    monkeypatch.setattr("time.sleep", lambda seconds: None)
...
_BRUTE_FORCE_LINES = [
    f"Mar 25 10:00:0{i} debian sshd[200{i}]: Failed password for invalid "
    f"user admin from 185.10.10.1 port 4000{i} ssh2"
    for i in range(8)
]


def test_daemon_tick_ingests_journald_and_alerts_on_new_log_findings(
    monkeypatch, tmp_path
):
    """Daemon ticks should ingest journald and alert only on new findings."""
    db_path = tmp_path / "test.db"
    config_path = tmp_path / "config.yaml"
    config_path.write_text(f"""
daemon:
...
""")
    ...
    monkeypatch.setattr(
        ...
        lambda unit_map, since, logger, timeout_seconds, identifier_map=None: {
            "auth": list(_BRUTE_FORCE_LINES)
        },
    )
    # The sample lines carry old timestamps; bypass the incremental window.
    monkeypatch.setattr(
        "traxerax_lite.main._parse_since",
        lambda value, connection: datetime(2000, 1, 1, tzinfo=timezone.utc),
    )
    dispatched = []
    monkeypatch.setattr(
        "traxerax_lite.main.dispatch_alerts",
        lambda **kwargs: dispatched.append(kwargs),
    )

    _run_daemon_with_stubbed_scheduler(
        monkeypatch, config_path, db_path, ticks=2
    )
    ...
    assert events == len(_BRUTE_FORCE_LINES)
    assert findings > 0
    assert len(dispatched) == 2
    assert dispatched[0]["log_findings"], "first tick should alert on findings"
    assert dispatched[1]["log_findings"] == [], "second tick must stay silent"


def test_daemon_tick_skips_log_ingestion_when_disabled(monkeypatch, tmp_path):
    """run_log_ingestion: false must keep journald out of the daemon tick."""
    db_path = tmp_path / "test.db"
    config_path = tmp_path / "config.yaml"
    config_path.write_text("""
daemon:
...
""")
    ...
    def fake_collect(unit_map, since, logger, timeout_seconds, identifier_map=None):
        called.append(True)
        return {"auth": list(_BRUTE_FORCE_LINES)}

    monkeypatch.setattr(
        "traxerax_lite.main.collect_journald_events", fake_collect
    )

    _run_daemon_with_stubbed_scheduler(monkeypatch, config_path, db_path)
