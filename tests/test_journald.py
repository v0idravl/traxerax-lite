...
def test_main_journal_ingestion_produces_findings(monkeypatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        monkeypatch.setattr(
            ...
            lambda unit_map, since, logger, timeout_seconds=30, identifier_map=None: journald_lines,
        )
...
def test_collect_merges_identifier_lines_into_source(monkeypatch) -> None:
    """Syslog identifiers (sudo) should be collected via -t and merged."""
    sudo_entry = json.dumps(
        {
            "MESSAGE": (
                "   alice : TTY=pts/0 ; PWD=/home/alice ; USER=root ; "
                "COMMAND=/usr/bin/id"
            ),
            "__REALTIME_TIMESTAMP": "1774523100000000",
            "_HOSTNAME": "debian",
    ...
    def fake_run(argv, **kwargs):
        calls.append(argv)
        stdout = sudo_entry if "-t" in argv else ""
    ...
    lines = collect_journald_events(
        unit_map={"auth": ("ssh",)},
        since=None,
        logger=_logger(),
        identifier_map={"auth": ("sudo",)},
    )

    ident_argv = next(argv for argv in calls if "-t" in argv)
    assert "sudo" in ident_argv
    assert "-u" not in ident_argv
    assert len(lines["auth"]) == 1
    assert "sudo" in lines["auth"][0]
    assert "COMMAND=/usr/bin/id" in lines["auth"][0]
