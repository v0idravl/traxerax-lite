...
def collect_journald_events(
    ...
    identifier_map: Optional[dict[str, tuple[str, ...]]] = None,
) -> dict[str, list[str]]:
    ...
    ``identifier_map`` collects additional entries by syslog identifier
    (``journalctl -t``) instead of unit — needed for programs like sudo
    whose log entries live under user session units, not a dedicated unit.
    Identifier-collected lines are merged into their source's list.
    """
    ...
    for source, identifiers in (identifier_map or {}).items():
        if not identifiers:
            continue
        lines = _collect_unit_lines(
            source, identifiers, since, timeout_seconds, logger, match_flag="-t"
        )
        if lines:
            lines_by_source.setdefault(source, []).extend(lines)
