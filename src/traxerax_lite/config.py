...
DEFAULT_FINDING_SEVERITIES = {
    ...
    "repeated_sudo_auth_failures": "medium",
}
...
class DetectionSettings:
    ...
    sudo_failed_auth_threshold: int = 3
    sudo_failure_window_seconds: int = 900
...
class AuditSettings:
    ...
    check_severities: dict[str, str] = field(
        default_factory=lambda: {
            ...
            "firewall_inactive": "medium",
            "snap_confinement": "medium",
            "flatpak_permissions": "medium",
        }
    )
...
class JournaldSettings:
    ...
    # Syslog identifiers collected via `journalctl -t` in addition to units;
    # sudo logs live under user session units, not a sudo unit, so unit-based
    # collection alone never sees them. gdm-password, sddm-helper, and login
    # cover local graphical/TTY login failures on desktops.
    identifiers: dict[str, tuple[str, ...]] = field(
        default_factory=lambda: {
...
class ChangeSettings:
    ...
    xdg_autostart: bool = True
    systemd_user_units: bool = True
    usb_devices: bool = True
    browser_extensions: bool = True
    ignored_listen_ports: tuple[int, ...] = ()
    ignored_kernel_modules: tuple[str, ...] = ()
    ignored_usb_devices: tuple[str, ...] = ()
    ignored_browser_extensions: tuple[str, ...] = ()
...
class AlertSettings:
    ...
    max_drops: int = 100
    # Desktop user that receives notify-send alerts when the process runs as
    # root (e.g. the systemd daemon); empty means notify as the current user.
    notify_user: str = ""
...
def load_detection_settings(config: dict[str, Any]) -> DetectionSettings:
    ...
    settings = DetectionSettings(
        ...
        sudo_failed_auth_threshold=int(
            thresholds.get("sudo_failed_auth", 3)
        ),
        sudo_failure_window_seconds=int(
            windows.get("sudo_failed_auth_seconds", 900)
        ),
...
def load_journald_settings(config: dict[str, Any]) -> JournaldSettings:
    ...
    identifiers = journald_config.get("identifiers")
    identifier_map: dict[str, tuple[str, ...]] = dict(default.identifiers)
    if isinstance(identifiers, dict):
        identifier_map = {
            str(source): tuple(str(identifier) for identifier in ident_list)
            for source, ident_list in identifiers.items()
...
def load_daemon_settings(config: dict[str, Any]) -> DaemonSettings:
    ...
    return DaemonSettings(
        ...
        run_log_ingestion=bool(
            daemon_config.get("run_log_ingestion", default.run_log_ingestion)
        ),
...
def load_change_settings(config: dict[str, Any]) -> ChangeSettings:
    ...
    return ChangeSettings(
        ...
        xdg_autostart=bool(
            change_config.get("xdg_autostart", default.xdg_autostart)
        ),
        systemd_user_units=bool(
            change_config.get("systemd_user_units", default.systemd_user_units)
        ),
        usb_devices=bool(
            change_config.get("usb_devices", default.usb_devices)
        ),
        browser_extensions=bool(
            change_config.get("browser_extensions", default.browser_extensions)
        ),
        ...
        ignored_usb_devices=tuple(
            str(name)
            for name in change_config.get("ignored_usb_devices", ())
        ),
        ignored_browser_extensions=tuple(
            str(name)
            for name in change_config.get("ignored_browser_extensions", ())
        ),
    )
...
def load_alert_settings(config: dict[str, Any]) -> AlertSettings:
    ...
    return AlertSettings(
        ...
        notify_user=str(alert_config.get("notify_user", default.notify_user)),
    )
