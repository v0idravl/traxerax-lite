...
from traxerax_lite.installer import run_setup
...
def main() -> None:
    ...
    if args.setup:
        # Self-contained: generates its own deployment config and opens its
        # own database, so it runs before config/DB initialization above the
        # normal mode dispatch and never creates a stray cwd database.
        run_setup(logger)
        return
...
def _run_log_ingestion(
    ...
    if args.journal:
        if journald_settings.enabled:
            journald_lines = collect_journald_events(
                ...
                identifier_map=journald_settings.identifiers,
            )
        else:
            logger.warning(
    ...
    parsed_count, finding_count, _new_findings = _process_ingested_records(
        connection=connection,
...
def _run_daemon_mode(
    ...
    def tick() -> None:
        ...
        # Incremental journald ingestion so log-based detection (SSH
        # brute-force, fail2ban, sprayed users, ...) also runs automatically.
        new_log_findings: list[Finding] = []
        if daemon_settings.run_log_ingestion and journald_settings.enabled:
            try:
                since_time = _parse_since("last-run", connection)
                journald_lines = collect_journald_events(
                    unit_map=journald_settings.units,
                    since=since_time,
                    logger=logger,
                ...
                _, _, new_log_findings = _process_ingested_records(
                    connection=connection,
                    ordered_records=ordered_records,
                    state=DetectionState.from_settings(detection_settings),
                    baseline_settings=baseline_settings,
                ...
                # Fail-soft: a journald hiccup must not kill the tick.
                logger.warning("daemon log ingestion failed: %s", exc)

        total_findings = (
            len(audit_findings)
            + len(rootkit_findings)
            + len(integrity_findings)
            + len(new_log_findings)
        )
        ...
        _finalize_run_alerts(
            ...
            integrity_findings=new_integrity_findings,
            rootkit_findings=new_rootkit_findings,
            log_findings=new_log_findings,
        )
        if not daemon_settings.quiet_when_clean or total_findings > 0:
            if new_log_findings:
                logger.info("[LOG FINDINGS] new=%d", len(new_log_findings))
            report = format_combined_report(
                run_record=run_record,
                audit_findings=audit_findings,
                integrity_findings=integrity_findings,
...
def _collect_normalized_events(
    ...
    def parse_auth_or_sudo(line: str) -> Event | EnforcementAction | None:
        # auth.log and the journald "auth" source also carry sudo records
        # (command invocations and PAM failures); try the sudo parser when
        # the ssh parser does not match.
        return parse_auth_line(
            line, year=year, local_timezone=local_timezone
        ) or parse_sudo_line(line, year=year, local_timezone=local_timezone)

    collect_from_log(auth_log, parse_auth_or_sudo)
    collect_from_log(
        fail2ban_log,
...
def _process_ingested_records(
    ...
    """Persist ingested records, run detection, and rebuild incidents.

    Shared by the CLI log-ingestion path and the daemon tick. Returns
    (parsed_count, finding_count, new_findings); ``new_findings`` holds only
    findings whose hashes were new to the database, suitable for alerting.
    """
    ...
    for record in ordered_records:
        ...
        for finding in findings:
            finding_count += 1
            if insert_finding(connection, finding):
                new_findings.append(finding)

    rebuild_incidents(connection, detection_settings)
    return parsed_count, finding_count, new_findings
...
def _seed_detection_state_from_history(
    ...
    max_window_seconds = max(
        ...
        state.sudo_failure_window_seconds,
    )
