...
# severity, label, escalate on suspicious exec path)
...
# USB interface class codes (hex strings as found in sysfs).
_USB_INTERFACE_HID = "03"
_USB_INTERFACE_MASS_STORAGE = "08"

# Browser extension permissions that make a new or changed installation
# worth a closer look (local heuristics; no blocklists or network data).
_HIGH_RISK_EXTENSION_PERMISSIONS = frozenset({
    "nativeMessaging",
    "debugger",
    "proxy",
    "webRequestBlocking",
...
def detect_host_changes(
    ...
    for record_type, (
        toggle, new_type, changed_type, severity, label, escalate
    ) in _PATH_TRACKED_CATEGORIES.items():
    ...
    if settings.browser_extensions:
        findings.extend(
            _detect_browser_extension_changes(
                connection, run_id, timestamp, host_records, settings
            )
        )
...
def _detect_path_changes(
    ...
    severity: str,
    label: str,
    escalate_suspicious: bool = False,
) -> list[RootkitFinding]:
    ...
    for record in _records_of_type(host_records, record_type):
        ...
        suffix = ""
        if escalate_suspicious and _references_suspicious_exec_path(record.data):
            record_severity = "high"
            suffix = " (references a temporary/writable path)"
        ...
        if known_hashes is None:
            findings.append(
                _finding(
                    ...
                    message=f"New {label}: {path}{suffix}",
                    confidence=0.75,
                    remediation=(
            ...
            findings.append(
                _finding(
                    ...
                    severity=record_severity,
                    message=f"{label[:1].upper()}{label[1:]} changed: {path}{suffix}",
                    confidence=0.75,
                    remediation=(
...
def _detect_usb_changes(
    ...
    """Flag new USB devices and identity changes to known devices.

    Severity follows the interface classes the device presents: a new
    device with a HID (keyboard) interface is the BadUSB pattern (high),
    new mass storage is medium, anything else is low. Devices named in
    ``changes.ignored_usb_devices`` (matched against the
    vendor:product:serial identity) are suppressed.
    """
    hashes_by_name = _hashes_by_name(connection, "usb_device")
    ignored = set(settings.ignored_usb_devices)

    findings: list[RootkitFinding] = []
    for record in _records_of_type(host_records, "usb_device"):
        ...
        if name in ignored:
            continue
        classes = set(record.data.get("interface_classes") or [])
        product = record.data.get("product") or "unknown device"
        known_hashes = hashes_by_name.get(name)
        if known_hashes is None:
            if _USB_INTERFACE_HID in classes:
                severity = "high"
                message = (
                    f"New USB device with HID interface (possible BadUSB): "
                    f"{product} ({name})"
                )
            elif _USB_INTERFACE_MASS_STORAGE in classes:
                severity = "medium"
                message = f"New USB mass-storage device: {product} ({name})"
            else:
                severity = "low"
                message = f"New USB device: {product} ({name})"
            findings.append(
                _finding(
                    run_id,
                    timestamp,
                    finding_type="host_change_new_usb_device",
                    severity=severity,
                    message=message,
                    confidence=0.7,
                    remediation=(
                        "Verify you plugged this device in. A device that "
                        "presents keyboard (HID) interfaces unexpectedly can "
                        "type attacker keystrokes (BadUSB)."
                    ),
                    record=record,
                )
            )
        elif record.record_hash not in known_hashes:
            severity = "high" if _USB_INTERFACE_HID in classes else "medium"
            findings.append(
                _finding(
                    run_id,
                    timestamp,
                    finding_type="host_change_usb_device_changed",
                    severity=severity,
                    message=(
                        f"USB device identity changed: {product} ({name}); "
                        "its interfaces or attributes differ from the baseline"
                    ),
                    confidence=0.75,
                    remediation=(
                        "Compare the device's interface classes against the "
                        "baseline; re-flashed firmware can turn a benign "
                        "device malicious."
                    ),
                    record=record,
                )
...
def _detect_browser_extension_changes(
    connection: sqlite3.Connection,
    run_id: str,
    timestamp: datetime,
    host_records: list[HostStateRecord],
    ...
    """Flag newly installed or changed browser extensions.

    New extensions are medium when sideloaded (not from the vendor store)
    or requesting high-risk permissions, low otherwise; changes to known
    extensions are low (version churn is normal). Extensions named in
    ``changes.ignored_browser_extensions`` (matched against the
    user:browser:extension_id identity) are suppressed.
    """
    hashes_by_name = _hashes_by_name(connection, "browser_extension")
    ignored = set(settings.ignored_browser_extensions)

    findings: list[RootkitFinding] = []
    for record in _records_of_type(host_records, "browser_extension"):
        name = record.data.get("name")
        if name is None:
            continue
        name = str(name)
        if name in ignored:
            continue
        display = record.data.get("display_name") or record.data.get(
            "extension_id"
        )
        known_hashes = hashes_by_name.get(name)
        if known_hashes is None:
            risk_reasons = _extension_risk_reasons(record.data)
            severity = "medium" if risk_reasons else "low"
            suffix = f" ({'; '.join(risk_reasons)})" if risk_reasons else ""
            findings.append(
                _finding(
                    run_id,
                    timestamp,
                    finding_type="host_change_new_browser_extension",
                    severity=severity,
                    message=f"New browser extension: {display} [{name}]{suffix}",
                    confidence=0.65,
                    remediation=(
                        "Confirm the extension was installed intentionally; "
                        "review its permissions in the browser and remove it "
                        "if unrecognized."
                    ),
                    record=record,
                )
            ...
            findings.append(
                _finding(
                    run_id,
                    timestamp,
                    finding_type="host_change_browser_extension_changed",
                    severity="low",
                    message=(
                        f"Browser extension changed: {display} [{name}]; "
                        "version, state, or permissions differ from the baseline"
                    ),
                    confidence=0.6,
                    remediation=(
                        "Review the extension's current permissions; "
                        "unexpected permission growth can indicate a "
                        "compromised or sold extension."
                    ),
                    record=record,
                )
            )
...
def _extension_risk_reasons(data: dict[str, Any]) -> list[str]:
    """Explain why a new extension record looks risky, if it does."""
    reasons: list[str] = []
    if not data.get("from_store"):
        reasons.append("not installed from the vendor store")
    permissions = set(data.get("permissions") or [])
    risky = sorted(permissions & _HIGH_RISK_EXTENSION_PERMISSIONS)
    if risky:
        reasons.append(f"high-risk permissions: {', '.join(risky)}")
    return reasons
