...
def send_desktop_notification(
    ...
    When ``notify_user`` is set and the process runs as root (e.g. the
    systemd daemon), the notification is sent as that user via ``runuser``
    with ``DBUS_SESSION_BUS_ADDRESS`` pointing at their session bus — a root
    process cannot reach a user's session bus directly. Returns False (and
    logs at debug level) when notify-send is missing or the call fails;
    never raises.
    """
    ...
    if notify_user and os.geteuid() == 0:
        wrapped = _wrap_for_session_user(command, notify_user, logger)
        if wrapped is None:
            return False
        command = wrapped
    try:
...
def _wrap_for_session_user(
    command: list[str],
    notify_user: str,
    logger: logging.Logger,
) -> list[str] | None:
    """Wrap a notify-send command to run as the desktop user's session.

    Returns None (and logs at debug level) when the user or their session
    bus cannot be resolved; never raises.
    """
    import pwd

    try:
        uid = pwd.getpwnam(notify_user).pw_uid
    except KeyError:
        logger.debug("notify_user %r does not exist; skipping", notify_user)
        return None
    bus_path = Path(f"/run/user/{uid}/bus")
    if not bus_path.exists():
        logger.debug("no session bus at %s; skipping notification", bus_path)
        return None

    runuser = shutil.which(
        "runuser", path="/usr/sbin:/sbin:/usr/bin:/bin"
    )
    env_binary = shutil.which("env", path="/usr/bin:/bin")
    if runuser is None or env_binary is None:
        logger.debug("runuser/env not found; skipping desktop notification")
        return None
    return [
        runuser,
        "-u",
