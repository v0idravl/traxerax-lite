...
@_register("pending_security_updates")
def _check_pending_security_updates(
    run_id: str,
    timestamp: datetime,
    settings: AuditSettings,
) -> list[AuditFinding]:
    """Flag pending security updates on apt-based systems.

    Runs ``apt list --upgradable`` (resolved from fixed system paths), which
    reads only the local package lists — no network access. Non-apt systems
    and command failures produce no findings.
    """
    findings: list[AuditFinding] = []
    severity = settings.check_severities.get(
        "pending_security_updates", "medium"
    )

    apt = shutil.which("apt", path="/usr/bin:/bin")
    if apt is None:
        return findings
    try:
        result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if not stripped or stripped.startswith("Listing") or "/" not in stripped:
            continue
        upgradable.append(stripped.split("/", 1)[0])
        suite = stripped.split("/", 1)[1].split(None, 1)[0]
        if "security" in suite:
            security.append(stripped.split("/", 1)[0])

    # Routine updates are not findings; pending security updates are.
    if security:
        findings.append(
            AuditFinding(
                run_id=run_id,
                timestamp=timestamp,
                check_id="pending_security_updates",
                severity=severity,
                message=(
                    f"{len(security)} security update(s) pending "
                    f"({len(upgradable)} upgradable packages total)"
                ),
                resource=", ".join(security[:5]),
                remediation="Apply pending security updates (apt upgrade or unattended-upgrades).",
                confidence=0.9,
            )
        )
    return findings
...
_UFW_CONF_PATH = Path("/etc/ufw/ufw.conf")


@_register("firewall_inactive")
def _check_firewall_inactive(
    run_id: str,
    timestamp: datetime,
    settings: AuditSettings,
) -> list[AuditFinding]:
    """Flag hosts with no active firewall (ufw disabled, no nftables chains).

    Detection is local-only: ufw state is read from its config file and
    nftables state from ``nft list ruleset`` (resolved from fixed system
    paths). A host with neither is exposed by default.
    """
    severity = settings.check_severities.get("firewall_inactive", "medium")
    if _ufw_active() or _nftables_active():
        return []
    return [
        AuditFinding(
            run_id=run_id,
            timestamp=timestamp,
            check_id="firewall_inactive",
            severity=severity,
            message="No active host firewall detected (ufw disabled and no nftables base chains)",
            resource="firewall",
            remediation="Enable ufw (ufw enable) or configure nftables base chains with a default-drop policy.",
            confidence=0.7,
        )
    ]
...
def _ufw_active() -> bool:
    """Return True when ufw is configured as enabled."""
    try:
        if not _UFW_CONF_PATH.is_file():
            return False
        for line in _UFW_CONF_PATH.read_text(
...
def _nftables_active() -> bool:
    """Return True when the nftables ruleset contains base chains."""
    ...
    if result.returncode != 0:
        return False
    # Only base chains carry a "hook" keyword; their presence means traffic
    # is actually being filtered.
    return "hook" in result.stdout
...
@_register("snap_confinement")
def _check_snap_confinement(
    run_id: str,
    timestamp: datetime,
    settings: AuditSettings,
) -> list[AuditFinding]:
    """Flag snaps installed with classic or devmode confinement.

    Classic and devmode snaps run unsandboxed, which defeats the point of
    the snap confinement model. State is read from snapd's local state file
    (root-only); non-root runs fall back to the local ``snap list`` output.
    No network is involved either way.
    """
    findings: list[AuditFinding] = []
    severity = settings.check_severities.get("snap_confinement", "medium")

    for name, confinement in sorted(_snap_confinements().items()):
        if confinement not in ("classic", "devmode"):
            continue
        findings.append(
            AuditFinding(
                run_id=run_id,
                timestamp=timestamp,
                check_id="snap_confinement",
                severity=severity,
                message=(
                    f"Snap '{name}' uses {confinement} confinement "
                    "(unsandboxed)"
                ),
                resource=f"snap:{name}",
                remediation=(
                    "Prefer a strictly confined package; if this snap must "
                    "stay, understand that it has full host access."
                ),
                confidence=0.9,
                data={"snap": name, "confinement": confinement},
            )
        )
    return findings


def _snap_confinements() -> dict[str, str]:
    """Return snap name -> confinement from snapd state, or {} when absent."""
    state_path = Path("/var/lib/snapd/state.json")
    try:
        import json

        with state_path.open("r", encoding="utf-8", errors="replace") as handle:
            payload = json.load(handle)
        snaps = payload.get("data", {}).get("snaps", {})
        if isinstance(snaps, dict):
            result = {}
            for name, entry in snaps.items():
                if isinstance(entry, dict):
                    confinement = entry.get("confinement")
                    if confinement:
                        result[str(name)] = str(confinement)
            if result:
                return result
    except (OSError, ValueError):
        pass

    # Fallback for unprivileged runs: parse `snap list` (local binary).
    snap = shutil.which("snap", path="/usr/bin:/bin:/snap/bin")
    if snap is None:
        return {}
    try:
        result = subprocess.run(
    ...
    for line in result.stdout.splitlines()[1:]:
        ...
        if "classic" in notes:
            confinements[name] = "classic"
        elif "devmode" in notes:
            confinements[name] = "devmode"
        else:
            confinements[name] = "strict"
    return confinements


@_register("flatpak_permissions")
def _check_flatpak_permissions(
    run_id: str,
    timestamp: datetime,
    settings: AuditSettings,
) -> list[AuditFinding]:
    """Flag flatpak apps whose effective permissions punch out of the sandbox.

    Reads each app's local metadata keyfile plus any overrides (system and
    per-user) and flags the high-impact grants: full host/home filesystem
    access, arbitrary D-Bus session access, and unrestricted device access.
    All reads are local files; no flatpak CLI or network is involved.
    """
    findings: list[AuditFinding] = []
    severity = settings.check_severities.get("flatpak_permissions", "medium")

    for app_id, context in sorted(_flatpak_effective_contexts().items()):
        problems = _flatpak_risky_grants(context)
        if not problems:
            continue
        findings.append(
            AuditFinding(
                run_id=run_id,
                timestamp=timestamp,
                check_id="flatpak_permissions",
                severity=severity,
                message=(
                    f"Flatpak '{app_id}' has risky permissions: "
                    f"{'; '.join(problems)}"
                ),
                resource=f"flatpak:{app_id}",
                remediation=(
                    "Review with `flatpak info --show-permissions`; tighten "
                    "overrides so the app only gets the access it needs."
                ),
                confidence=0.8,
                data={"app": app_id, "problems": problems},
            )
        )
    return findings


_FLATPAK_METADATA_GLOB = "app/*/*/*/current/active/metadata"
_FLATPAK_SYSTEM_ROOT = Path("/var/lib/flatpak")


def _flatpak_effective_contexts() -> dict[str, dict[str, set[str]]]:
    """Merge flatpak metadata and override [Context] sections per app id."""
    contexts: dict[str, dict[str, set[str]]] = {}

    metadata_roots = [_FLATPAK_SYSTEM_ROOT]
    import pwd as _pwd

    for user in _pwd.getpwall():
        metadata_roots.append(
            Path(user.pw_dir) / ".local" / "share" / "flatpak"
        )

    for root in metadata_roots:
        for metadata in root.glob(_FLATPAK_METADATA_GLOB):
            try:
                app_id = metadata.relative_to(root / "app").parts[0]
            except (ValueError, IndexError):
                continue
            _merge_flatpak_keyfile(metadata, contexts.setdefault(app_id, {}))

    override_dirs = [_FLATPAK_SYSTEM_ROOT / "overrides"]
    for user in _pwd.getpwall():
        override_dirs.append(
            Path(user.pw_dir) / ".local" / "share" / "flatpak" / "overrides"
        )
    for override_dir in override_dirs:
        try:
            entries = sorted(override_dir.glob("*"))
        except OSError:
            continue
        for override in entries:
            ...
            _merge_flatpak_keyfile(
                override, contexts.setdefault(override.name, {})
            )

    return contexts


def _merge_flatpak_keyfile(
    path: Path,
    context: dict[str, set[str]],
) -> None:
    """Merge the [Context] key values of one flatpak keyfile into context."""
    import configparser
    ...
    if not parser.has_section("Context"):
        return
    for key, value in parser.items("Context"):
        values = {
            item for item in value.split(";") if item
        }
        context.setdefault(key, set()).update(values)


def _flatpak_risky_grants(context: dict[str, set[str]]) -> list[str]:
    """Describe the sandbox-breaking grants present in a flatpak context."""
    problems: list[str] = []
    filesystems = context.get("filesystems", set())
    if filesystems & {"host", "host-os"}:
        problems.append("full host filesystem access")
    elif "home" in filesystems:
        problems.append("full home directory access")
    sockets = context.get("sockets", set())
    if "session-bus" in sockets:
        problems.append("unrestricted D-Bus session access")
    if "all" in context.get("devices", set()):
        problems.append("unrestricted device access")
    return problems
