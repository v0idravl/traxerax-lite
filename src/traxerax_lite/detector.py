...
class DetectionState:
    ...
    sudo_failure_times: dict[str, deque[datetime]] = field(
        default_factory=lambda: defaultdict(deque)
    )
    sudo_threshold_alerted: set[str] = field(default_factory=set)
    ...
    def from_settings(cls, settings: DetectionSettings) -> "DetectionState":
        ...
        return cls(
            ...
            sudo_failed_auth_threshold=settings.sudo_failed_auth_threshold,
            sudo_failure_window_seconds=settings.sudo_failure_window_seconds,
...
def process_event(event: Event, state: DetectionState) -> list[Finding]:
    ...
    if event.source == "sudo" or event.event_type == "pam_auth_failure":
        # Sudo/PAM events have no src_ip; they are keyed by username instead.
        findings.extend(_process_sudo_event(event, state))
...
def _process_sudo_event(
    event: Event,
    state: DetectionState,
) -> list[Finding]:
    """Process a normalized sudo/PAM event (keyed by username, not IP)."""
    findings: list[Finding] = []
    ...
    if event.event_type in {
        ...
        failures = state.sudo_failure_times[user]
        failures.append(event.timestamp)
        _prune_datetimes(
            failures,
            event.timestamp,
        ...
        if (
            len(failures) >= state.sudo_failed_auth_threshold
            and user not in state.sudo_threshold_alerted
        ):
            state.sudo_threshold_alerted.add(user)
            service = event.service or "sudo"
            finding = _make_finding(
                state=state,
                finding_type=(
                    "repeated_sudo_auth_failures"
                    if service == "sudo"
                    else "repeated_local_auth_failures"
                ),
                message=(
                    f"Repeated {service} authentication failures for user "
                    f"{user} ({len(failures)} failures within "
                    f"{state.sudo_failure_window_seconds}s)"
                ),
                src_ip=None,
                timestamp=event.timestamp,
            )
